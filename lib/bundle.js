...
var SteinManager = function () {
  ...
  _createClass(SteinManager, [{
    key: 'newStein',
    value: function newStein(type, x, y, scaleX, scaleY, xLimit) {  // change from fillStatus to type  -1 for empty  0~3 for different cakes
      var imagePath = void 0;
      if (type == -1) {
        imagePath = 'assets/emptyStein.png';
      } else {
        imagePath = 'assets/fullStein' + type.toString()  + '.png';
      }
      var STEINS_DATA = {
        images: [imagePath],
      ...
      var _ref = [x, y, scaleX || STEIN.scaleX, scaleY || STEIN.scaleY, xLimit || STEIN.xLimit, type];
      STEIN.x = _ref[0];
      STEIN.y = _ref[1];
      STEIN.scaleX = _ref[2];
      STEIN.scaleY = _ref[3];
...
var Game = function () {
  ...
  _createClass(Game, [{
    ...
    value: function setup(buttonClicked) {
      ...
      document.getElementById('over-filled-canvas').focus();
      ...
      this.stage = new createjs.Stage('over-filled-canvas');
...
var CustomerManager = function () {
  ...
  },
  {
    ...
    value: function createCustomer() {
      ...
      var RANDOM_SPEED = (Math.floor(Math.random() * 5) + 5) * 3750;
      var RANDOM_BAR_INDEX = Math.floor(Math.random() * 4);

      var IMAGE_PATHS = ['assets/customer0.png', 'assets/customer1.png', 'assets/customer2.png', 'assets/customer3.png'];
      var FRAME_DIMENSIONS = [{ width: 80, height: 75 }];
      ...
      var SETTERS = ['speed', 'barIndex', 'x', 'y', 'scaleX', 'scaleY', 'tweens', 'scoreValue', 'type'];

      var VALUES = [RANDOM_SPEED, RANDOM_BAR_INDEX, _bar.BARS[RANDOM_BAR_INDEX].posXLimits[0], _bar.BARS[RANDOM_BAR_INDEX].startPosY + 5, _bar.BARS[RANDOM_BAR_INDEX].scale, _bar.BARS[RANDOM_BAR_INDEX].scale, [], 50, RANDOM_SELECTOR];
...
var Input = function () {
  function Input(player, timeouts, steins, stage) {
    ...
    this.stage = _ref2[2];
    this.steins = _ref2[3];
    this.steinType = -1    // -1 for empty  0~3 for different cakes
  ...
  _createClass(Input, [{
    ...
    value: function restartIdle(e) {
      ...
      if (e.keyCode === 37) {
        if(this.steinType !== -1){
          ...
          // this.inMotion = false;
          var NEW_FULL_STEIN = this.steins.newStein(this.steinType, _bar.BARS[this.barIndex].startPosX, _bar.BARS[this.barIndex].startPosY + 50, null, null, _bar.BARS[this.barIndex].posXLimits[0]);
    ...
    value: function fillStein() {
      ...
      if (true) {
        ...
        this.matchPlayerPositionWithBar();
        this.player.x += 30;
        this.steinType = this.barIndex
        // TODO  apply different fill ani according to the steinType (thus the animations prop of PLAYER_DATA should be changed)
...
var StatusScreens = function () {
  function StatusScreens(stage) {
    ...
    this.lives = [new _steins2.default().newStein(0, 663, 58, 0.75, 0.5), new _steins2.default().newStein(0, 693, 58, 0.75, 0.5), new _steins2.default().newStein(0, 723, 58, 0.75, 0.5)];
  }
      ...
      for (var i = 0; i < this.customers.length; i++) {
        for (var j = 0; j < this.fullSteins.length; j++) {
          if (this.fullSteinHitCustomer(i, j)) {
            if(this.fullSteins[j].type === this.customers[i].type){
              ...
              this.fullSteins[j].x = 0;
              this.stage.removeChild(this.fullSteins[j]);
              this.customers[i].x = _bar.BARS[this.customers[i].barIndex].posXLimits[1] + 30;   // trigger customerIsAtTheFinishLine to be true
            }
          }
        }
      }
  ...
  }, 
  {
    ...
    value: function checkWhetherAllCustomersAreServedOut() {
      ...
      if (this.customers.length < 1) {
        this.freezeAnimationsAndInput();
        this.that.difficulty = this.that.difficulty / 1.25;
    ...
    value: function checkForFallingEmptySteins() {
      for (var i = 0; i < this.emptySteins.length; i++) {
        if (this.steinIsFalling(i, this.emptySteins)) {
          ...
          var FALLING_STEIN = this.steins.newStein(-1, this.emptySteins[i].x, this.emptySteins[i].y);
    ...
    value: function checkForFallingFullSteins() {
      for (var i = 0; i < this.fullSteins.length; i++) {
        if (this.steinIsFalling(i, this.fullSteins)) {
          this.freezeAnimationsAndInput();
          this.player.gotoAndPlay('scared');
          var FALLING_STEIN = this.steins.newStein(this.fullSteins[i].type, this.fullSteins[i].x, this.fullSteins[i].y);
          this.stage.addChild(FALLING_STEIN);
      ...
      if (ORIGINAL_X - 115 > _bar.BARS[customer.barIndex].posXLimits[0] - 35) {
        this.timeouts.push(setTimeout(function () {
          if (_this3.customers.indexOf(customer) !== -1) {
            var NEW_EMPTY_STEIN = _this3.steins.newStein(-1, customer.x + 50, customer.y + 40, null, null, _bar.BARS[customer.barIndex].posXLimits[1] + 90);
            customer.gotoAndPlay('walkAngry');
