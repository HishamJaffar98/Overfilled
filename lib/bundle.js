...
var Game = function () {
  ...
  _createClass(Game, [{
    ...
    value: function setup(buttonClicked) {
      ...
      clearInterval(this.recipeGenerator);
      ...
      this.recipeGenerator = setInterval(() => 
      {
      this.customerManager.createRecipe(this.stage);
      this.stage.setChildIndex(this.player,this.stage.getNumChildren()-1);
      },
      5000);
    }
...
var CustomerManager = function () {
  ...
  _createClass(CustomerManager, [{
    ...
    value: function createCustomer() {
      var RANDOM_SELECTOR = Math.floor(Math.random() * 9);
      var RANDOM_SPEED = (Math.floor(Math.random() * 5) + 5) * 1750;
      var RANDOM_BAR_INDEX = Math.floor(Math.random() * 4);

      var IMAGE_PATHS = ['assets/TempIngredients/Base/Base_Chocolate.png', 'assets/TempIngredients/Base/Base_Orange.png',
                         'assets/TempIngredients/Base/Base_Pink.png', 'assets/TempIngredients/Base/Base_Milk.png',
                         'assets/TempIngredients/Base/Base_Purple.png', 'assets/TempIngredients/Icing/Icing_Chocolate.png',
                         'assets/TempIngredients/Icing/Icing_Blue.png','assets/TempIngredients/Topping/Topping_Blueberry.png',
                         'assets/TempIngredients/Topping/Topping_Strawberry.png'];
      // var FRAME_DIMENSIONS = [{ width: 90, height: 75 }, { width: 80, height: 75 }];
      var FRAME_DIMENSIONS = [{ width: 32 , height: 32 }];
      ...
      var VALUES = [RANDOM_SPEED, RANDOM_BAR_INDEX, _bar.BARS[RANDOM_BAR_INDEX].posXLimits[0], _bar.BARS[RANDOM_BAR_INDEX].startPosY + 60, _bar.BARS[RANDOM_BAR_INDEX].scale, _bar.BARS[RANDOM_BAR_INDEX].scale, [], 50];
...

  {
   key: 'createRecipe',
    value: function createRecipe() {
      ...
      var RANDOM_SPEED = 7 * 1750;
      var RANDOM_BAR_INDEX = Math.floor(Math.random() * 4);

      var IMAGE_PATHS = ['assets/TempFullRecipes/Cake0.png', 'assets/TempFullRecipes/Cake1.png', 'assets/TempFullRecipes/Cake2.png','assets/TempFullRecipes/Cake3.png'];
      // var FRAME_DIMENSIONS = [{ width: 90, height: 75 }, { width: 80, height: 75 }];
      var FRAME_DIMENSIONS = [{ width: 32, height: 32 }];
      ...
      var VALUES = [RANDOM_SPEED, 0, _bar.BARS[0].posXLimits[0], _bar.BARS[3].startPosY-70, 1.5, 1.5, [], 50];

      for (var i = 0; i < SETTERS.length; i++) {
        CUSTOMER[SETTERS[i]] = VALUES[i];
      }

      CUSTOMER.tweens.push(createjs.Tween.get(CUSTOMER, { loop: false }).to({ x: _bar.BARS[0].posXLimits[1] + 30 }, RANDOM_SPEED, createjs.Ease.getPowInOut(1)));
