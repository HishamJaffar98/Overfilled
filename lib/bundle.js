...
var Game = function () {
	...
	_createClass(Game, [{
		...
		value: function SpawnCustomers() {
			...
			var f = function f() {
				var customersToSpawn = 1 + Math.floor(Math.random() * _this.screenManager.levelNum);
...
var Input = function () {
	function Input(player, stage) {
		...
		this.cakes = [];
		this.sfx_CakeSend = new Audio();
		this.sfx_CakeSend.src = "Audio/OverFilled_SFX_CakeSend.mp3";
		this.sfx_PlayerMove = new Audio();
		this.sfx_PlayerMove.src = "Audio/OverFilled_SFX_PlayerMove.mp3";
	}

	_createClass(Input, [{
		key: "MovePlayer",
		value: function MovePlayer(e) {
			if (this.player.playerSprite.itemHeld === "undisturbed") return;
			if (e.keyCode === 38) {
				this.MoveUp();
...
var ScreenManager = function () {
	...
	_createClass(ScreenManager, [{
		...
		value: function ShowLivesAndScore(lives, currentScore) {
			...
			this.stage.addChild(this.highScoreText, this.currentScoreText, this.livesText);
			this.livesLeft = lives;
			for (var i = 0; i < 3; i++) {
				this.stage.removeChild(this.lives[i]);
				if (this.livesLeft > i) this.stage.addChild(this.lives[i]);
			}
		}
	}, {
		key: 'LifeLost',
		...
		value: function PauseGame() {
			...
			if (this.game.lives > 0) {
				...
				setTimeout(function () {
					...
					setTimeout(function () {
						_this.game.score = 0;
						_this.game.screenManager.levelNum = 1;
						_this.game.Start();
					}, 2000);
				}, 2000);
			} else {
				setTimeout(function () {
					...
					setTimeout(function () {
						...
						// this.game.Start();
						_this.stage.removeAllChildren();
						_this.game.screenManager.DrawLevel("Controls");
						_this.game.screenManager.DrawLevel("BackButton");
					}, 2000);
				}, 2000);
			}
		}
